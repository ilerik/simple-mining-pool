...
fn echo(req: Request<Body>) -> BoxFut {
    ...
    match (req.method(), req.uri().path()) {
        ...
        (&Method::GET, "/") => {
            *response.body_mut() = Body::from("Welcome to simple mining pool. Use /signup for registering new user identity.");
        }
        ...
        // Signup new user and verify his identity
        (&Method::POST, "/signup") => {
            let reversed = req.into_body().concat2().map(move |chunk| {
                let body = chunk.iter().rev().cloned().collect::<Vec<u8>>();

                // Extract request data
                // let json: serde_json::Value = if let Ok(j) = serde_json::from_slice(body.as_ref()) {
                //     // Log output
                //     println!("Incoming signup request payload: {:?}", j);
                ...
                //     let body: ResponseStream = Box::new(Body::from(bad_request));
                //     return Response::new()
